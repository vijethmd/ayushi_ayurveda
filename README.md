...
`render.yaml` is a Blueprint — Render reads it and configures the service. The database stays on
Aiven; only the web service is defined here.

1. Push this branch, then on Render: **New → Blueprint**, pick the repo, approve the plan.
2. Set the four secrets Render prompts for (they are `sync: false`, so never committed):
   `DATABASE_URL`, `EMAIL_USER`, `EMAIL_PASS`, `EMAIL_FROM`, `GEMINI_API_KEY`.
...
Render wipes the filesystem on every deploy, which would orphan uploaded reports and discharge
summaries. `STORAGE_DRIVER` decides where the bytes go:

| value | behaviour |
|---|---|
| `disk` | files under `backend/uploads/` — the default locally |
| `db` | bytes in `Attachments.content` — set for Render, survives redeploys |

Existing rows keep working either way: each carries a `storage` column, and the download route
serves from whichever the row used. Run `node backend/sql/migrate_attachment_storage.js` once
against the host before deploying.

### Free tier behaviour

A free Render service sleeps after ~15 minutes idle, so the first request afterwards takes
roughly 50 seconds while it wakes. Sessions are in-process memory, so a restart signs users out —
fine for one instance, worth moving to a shared store if you ever scale beyond one.
