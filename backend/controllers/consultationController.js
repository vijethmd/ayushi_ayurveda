...
const { removeFile, UPLOAD_DIR, DRIVER } = require('../middleware/upload');
...
const saveAttachments = async (files, { patientId, consultationId, treatmentId, kind, userId, label }) => {
  for (const f of files || []) {
    // multer gives us f.buffer under the db driver and f.filename under disk.
    const toDb = DRIVER === 'db';
    await db.query(
      `INSERT INTO Attachments
        (patient_id, consultation_id, treatment_id, kind, label,
         original_name, stored_name, mime_type, size_bytes, uploaded_by, storage, content)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
      [patientId, consultationId || null, treatmentId || null, kind, label || null,
       f.originalname, toDb ? null : f.filename, f.mimetype, f.size, userId || null,
       toDb ? 'db' : 'disk', toDb ? f.buffer : null]);
  }
};
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    (req.files || []).forEach(f => removeFile(f.filename));  // no-op under the db driver
...
exports.download = async (req, res) => {
  try {
    ...
    if (a.storage === 'db') {
      if (!a.content) return res.status(404).send('File content is missing');
